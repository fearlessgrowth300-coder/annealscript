use intentpm::lockfile::{LockEntry, Lockfile};
use intentpm::manifest::{Kind, Version};
use intentpm::registry::{self, Registry};
...
fn run() -> Result<(), String> {
    ...
    let registry = Registry::new(env::var("INTENTPM_REGISTRY").unwrap_or_else(|_| "registry".to_string()));
    let lock_path = PathBuf::from("intent.lock");

    match args.get(1).map(String::as_str) {
        Some("publish") => {
            let src = PathBuf::from(args.get(2).ok_or("usage: intentpm publish <src_dir> <name> <version> <lib|model> [k=v ...]")?);
        ...
        Some("add") => {
            let name = args.get(2).ok_or("usage: intentpm add <name> [version]")?;
            ...
            let dest = PathBuf::from("intent_packages").join(name).join(version.to_string());
            ...
            lock.save(&lock_path)?;
            println!("added {name}@{version} [{}] -> intent_packages/{name}/{version}", manifest.kind.as_str());
        }
        Some("list") => {
            let lock = Lockfile::load(&lock_path);
            if lock.entries.is_empty() {
                println!("no packages installed (see intent.lock)");
            }
        ...
        _ => {
            eprintln!("usage: intentpm <publish|add|list> ...");
            std::process::exit(2);
        }
    }
    Ok(())
